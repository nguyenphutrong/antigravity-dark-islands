...
### Nix Flake (Legacy VS Code/VSCodium)

The Nix flake in this repo currently targets VS Code/VSCodium packages (legacy path), not Antigravity.
...
1. Open **Extensions** in Antigravity: (`Cmd+Shift+X` / `Ctrl+Shift+X`)
...
Copy the contents of `settings.json` from this repo into your Antigravity settings:
...
3. Antigravity will reload

> **Note:** You may see a "corrupt installation" warning after enabling. This is expected since Custom UI Style injects CSS into Antigravity. Click the gear icon on the warning and select **Don't Show Again**.
